...
编辑区使用固定四列的“输入 / 当前”两行矩阵：同类控件严格上下对齐，支持试音后写入单音 / 和弦或休止符，并通过最近试音替换当前音高。删除、定位与回到末尾在矩阵右侧跨两行居中；窄屏仅横向滚动，不增加编辑区高度。撤销重做、本地保存与 JSON 导入导出继续保留。
输入音与待写入谱面预览使用主题强调色，当前音与谱面选中事件使用主题蓝色，普通已写入内容保持原谱面色。
回放控制位于乐谱下方，支持单按钮播放 / 暂停和进度条拖动；BPM 可直接输入或每次 ±10，编辑光标与播放位置独立。
