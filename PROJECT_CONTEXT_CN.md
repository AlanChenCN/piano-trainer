...
电脑键盘物理映射范围为 `A` 到 `;`，默认覆盖 C4-E5，可通过 Toolbar 左右箭头切换为 C3-E4 或 C5-E6。显示出的其余琴键暂时不绑定电脑按键，后续由 MIDI 输入补充。
...
Input Layer 不感知 Keyboard、Octave 或具体输入来源，便于未来接入 MIDI、
Playback 等输入源。
